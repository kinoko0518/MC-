...
native_command = ["advancement", "attribute", "execute", "bossbar", "clear",\
                "clone", "damage", "data", "datapack", "debug", "defaultgamemode",\
                "difficulty", "effect", "me", "enchant", "experience",\
                "xp", "fill", "fillbiome", "forceload", "function",\
                "gamemode", "gamerule", "give", "help", "item",\
                "kick", "kill", "list", "locate", "loot",\
                "msg", "tell", "w", "particle", "place",\
                "playsound", "random", "reload", "recipe", "return",\
                "ride", "say", "schedule", "scoreboard", "seed",\
                "setblock", "spawnpoint", "setworldspawn", "spectate", "spreadplayers",\
                "stopsound", "summon", "tag", "team", "teammsg",\
                "tm", "teleport", "tp", "tellraw", "tick",\
                "time", "title", "trigger", "weather", "worldborder",\
                "jfr", "publish"]
...
def compile(_raw_code:str, current_dir:str):
    ...
    if not _raw_code.replace("    ", "") == "":
        ...
        if _raw[0].startswith("#"):
            add_code(_raw_code)
        elif _raw[0] in native_command:
            add_code(_raw_code)
        else:
            ...
            def get_defined_function(function_name:str):
                for _i in range(len(functions)):
                    if functions[_i][0] == function_name:
                        return _i
                return -1
            ...
            # 関数を変換
            print(_raw_code.rsplit("(")[0])
            print(not get_defined_function(_raw_code.rsplit("(")[0]) == -1, r"(.)" in _raw, _raw_code.endswith(")"))
            if not get_defined_function(_raw_code.rsplit("(")[0]) == -1 and r"(.)" in _raw and _raw_code.endswith(")"):
                __function__ = functions[get_defined_function(_raw_code.rsplit("(")[0])]
                __arguments__ = r"(.)"[1:-2].split(",")
                print(__function__, __arguments__)
                if len(__function__[2]) == len(__arguments__):
                    add_code(__function__[1] + " ".join(arguments))
                elif len(__function__[2]) <= len(__arguments__):
                    return_syntax_error("Too many arguments")
                elif len(__function__[2]) >= len(__arguments__):
                    return_syntax_error("Too few arguments")



            # 代入の処理
            if len(_raw) > 2 and _raw[1] == "=":
            ...
            if ":" in _raw[-1]:
                ...
                if _raw[0] == "func":
                    arguments = r"(.)"[1:-2].split(",")
                    functions.append([_raw_code.rsplit("(")[0], __result_in_indent__, arguments])
...
if raw_code_path.name.split("\\")[-1].split(".")[-1] == "mcpp": # 拡張子が.mcppか確認する
    while pointer in range(len(raw_code)):
        __compiled__ = compile(raw_code[pointer], compiled_folder)
